  ...
  tryCatch({
    ...
    if (httr::status_code(response) %in% c(200, 302)) {
      return(list(
        success = TRUE,
        message = paste(
          "✅ SUCCESS! Your quiz has been submitted!",
          "\n\nWhat happens next:",
          "• Your instructor has received your answers",
          "• Your submission was recorded at:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
          "• Module completed:", tutorial_id,
...
observeEvent(input$submit, {
  ...
  if (is.null(github_username) || github_username == "") {
    shiny::showNotification("❌ Please enter your GitHub username first before submitting.", type = "error", duration = NULL)
    output$submission_status <- renderUI({
      div(class = "alert alert-warning", style = "margin-top: 15px;",
          "❌ Please enter your GitHub username first before submitting.")
    })
    return()
  }
  ...
  # Show results
  if (result$success) {
    shiny::showNotification("🎉 Quiz submitted successfully!", type = "message", duration = NULL)
    output$submission_status <- renderUI({
      div(class = "alert alert-success", style = "margin-top: 15px;",
          h4("🎉 SUCCESS! Your quiz has been submitted!"),
    ...
    shiny::showNotification("❌ Submission failed", type = "error", duration = NULL)
    output$submission_status <- renderUI({
